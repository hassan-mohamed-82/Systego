...
export const createPopup = async (req: Request, res: Response) => {
  const { title_ar, title_En, description_ar, description_En, image, link } =
    req.body;
  if (!title_ar || !title_En || !description_ar || !description_En || !link)
    throw new BadRequest("All fields are required");
  const image_url = await saveBase64Image(
    image,
    Date.now().toString(),
    req,
    "popup",
...
export const updatePopup = async (req: Request, res: Response) => {
  ...
  const popup = await PopupModel.findByIdAndUpdate(id, updateData, {
    new: true,
  });
  if (!popup) throw new NotFound("Popup not found");
