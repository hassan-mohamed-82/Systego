...
export const appSettingValidationSchema = Joi.object({
  ...
  templateSlug: Joi.string().required().description("Slug of the template"),
  ...
  logo: Joi.string().allow(null, "").optional().description("Store logo as base64 or URL"),
  ...
  sections: Joi.array()
    .items(
      Joi.object({
        key: Joi.string().required().description("Unique key for the section"),
        enabled: Joi.boolean()
          .default(true)
