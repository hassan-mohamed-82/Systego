...
const Booking_js_1 = require("../models/schema/admin/Booking.js");
const products_js_1 = require("../models/schema/admin/products.js");
const product_price_js_1 = require("../models/schema/admin/product_price.js");
node_cron_1.default.schedule("0 * * * *", async () => {
    try {
        console.log("⏰ Checking pending bookings...");
        const now = new Date();
        const pendingBookings = await Booking_js_1.BookingModel.find({ status: "pending" });
        for (const booking of pendingBookings) {
            ...
            if (diffDays >= booking.number_of_days) {
                ...
                if (booking.ProductId) {
                    const product = await products_js_1.ProductModel.findById(booking.ProductId);
                    if (product) {
                        product.quantity += 1;
                        await product.save();
                ...
                if (booking.option_id) {
                    const option = await product_price_js_1.ProductPriceOptionModel.findById(booking.option_id);
                    if (option) {
                        const price = await product_price_js_1.ProductPriceModel.findById(option.product_price_id);
                        if (price) {
                            price.quantity += 1;
                            await price.save();
