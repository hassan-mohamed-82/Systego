...
// ============================================
const drawLabelRotated = async (doc, data, x, y, pdfWidth, // العرض الفعلي في الـ PDF (30mm)
pdfHeight, // الارتفاع الفعلي في الـ PDF (50mm)
config) => {
    ...
    // حساب ارتفاع كل عنصر
    const totalElements = (config.showBusinessName && data.businessName ? 1 : 0) +
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let i = 0; i < totalLabels; i++) {
                const x = 0;
                const y = pdfHeight * i;
                await drawLabelRotated(doc, labelsData[i], x, y, pdfWidth, pdfHeight, labelConfig);
            }
            doc.end();
        }
        catch (error) {
