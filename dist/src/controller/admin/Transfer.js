...
const createTransfer = async (req, res) => {
    ...
    if (!productId && !categoryId && !productCode)
        throw new BadRequest_1.BadRequest("Please provide productId or categoryId or productCode");
    ...
    const productInWarehouse = await Product_Warehouse_1.Product_WarehouseModel.findOne({ productId, warehouseId: fromWarehouseId });
    if (!productInWarehouse) {
        throw new index_1.NotFound("Product not found in the source warehouse");
    }
    if (productInWarehouse.quantity < quantity) {
        throw new BadRequest_1.BadRequest("Insufficient product quantity in the source warehouse");
    }
...
const getTransferById = async (req, res) => {
    ...
    (0, response_1.SuccessResponse)(res, { message: "Transfer retrieved successfully", transfer });
};
...
const markTransferAsReceived = async (req, res) => {
    ...
    if (transfer.status !== "pending")
        throw new BadRequest_1.BadRequest("Only pending transfers can be received");
    // تحقق إن المستودع المستقبل هو اللي بيعمل الاستلام
    if (transfer.toWarehouseId.toString() !== warehouseId)
        throw new BadRequest_1.BadRequest("Only the receiving warehouse can mark this transfer as received");
    // تحديث الحالة
