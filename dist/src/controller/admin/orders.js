...
const getOrdersReport = async (req, res) => {
    ...
    if (!startDate || !endDate) {
        throw new BadRequest_1.BadRequest("startDate and endDate are required");
    }
    ...
    // 📦 جلب الأوردرات
    // ═══════════════════════════════════════════════════════════
    const orders = await Sale_1.SaleModel.find(filter)
        .populate("customer_id", "name phone_number email")
    ...
    const allItems = await Sale_2.ProductSalesModel.find({
        sale_id: { $in: orderIds },
    })
