...
const bank_accounts_1 = require("../../models/schema/admin/bank_accounts");
...
const createBankAccount = async (req, res) => {
    ...
    const exists = await bank_accounts_1.BankAccountModel.findOne({ account_no });
    if (exists)
        throw new BadRequest_1.BadRequest("Account number already exists");
    // ✅ لو الحساب دا هو الـ default، خلّي الباقي false
    if (is_default) {
        await bank_accounts_1.BankAccountModel.updateMany({}, { is_default: false });
    }
    const bankAccount = await bank_accounts_1.BankAccountModel.create({
        account_no,
        name,
        initial_balance,
        is_default,
...
const getBankAccounts = async (req, res) => {
    const accounts = await bank_accounts_1.BankAccountModel.find();
...
const getBankAccountById = async (req, res) => {
    ...
    const account = await bank_accounts_1.BankAccountModel.findById(id);
    if (!account)
...
const updateBankAccount = async (req, res) => {
    ...
    const account = await bank_accounts_1.BankAccountModel.findByIdAndUpdate(id, req.body, { new: true });
    if (!account)
        throw new Errors_1.NotFound("Bank account not found");
...
const deleteBankAccount = async (req, res) => {
    ...
    if (!id)
        throw new BadRequest_1.BadRequest("Bank account ID is required");
    const account = await bank_accounts_1.BankAccountModel.findByIdAndDelete(id);
    if (!account)
        throw new Errors_1.NotFound("Bank account not found");
