...
exports.login = (0, express_async_handler_1.default)(async (req, res) => {
    const user = await Customer_1.CustomerModel.findOne({ email: req.body.email });
...
exports.getProfile = (0, express_async_handler_1.default)(async (req, res) => {
    const user = await Customer_1.CustomerModel.findById(req.user?.id).select('-password -v');
    if (!user) {
        throw new Errors_1.NotFound('User not found');
    }
